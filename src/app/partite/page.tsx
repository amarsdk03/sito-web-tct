...
export async function generateMetadata(): Promise<Metadata> {
    return dynamicMetadata(
        "Cerca partite",
        "Lista tutte le partite disputate nelle varie edizioni, categorie e gironi",
        `/partite`,
    );
}
 ...
 */
async function FixturesServer({idEdizione, idCategoria, nomeGirone}: paramProps) {
    ...
    try {
        const [listaCategorie, listaPartite] = await Promise.all([
            getListaCategorie(),
            getListaPartite(
                Number.parseInt(selectedTorneoId),
                Number.parseInt(idCategoria || "-1"),
                nomeGirone
            )
        ]);
    ...
    return (
        <>
            ...
            <Suspense fallback={
                ...
                <FixturesServer idEdizione={idEdizione} idCategoria={idCategoria} nomeGirone={nomeGirone} />
            </Suspense>
            <Footer/>
        </>
    );
